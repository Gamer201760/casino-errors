...
def test_balance_set_and_get_item(balance_store: CasinoBalance) -> None:
    balance_store['player_1'] = 100

    assert balance_store['player_1'] == 100


def test_balance_overwrite_value(balance_store: CasinoBalance) -> None:
    balance_store['player_1'] = 100
...
def test_balance_get_missing_key_raises_error(
    balance_store: CasinoBalance,
) -> None:
    with pytest.raises(KeyError):
        _ = balance_store['missing_player']


def test_balance_len(balance_store: CasinoBalance) -> None:
...
def test_balance_del_item(balance_store: CasinoBalance) -> None:
...
def test_balance_iter(balance_store: CasinoBalance) -> None:
    balance_store['p1'] = 10
    balance_store['p2'] = 20
    balance_store['p3'] = 30
